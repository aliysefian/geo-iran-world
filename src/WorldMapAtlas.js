...
const WorldMap = () => {
  ...
  const handleCountryHover = countryIndex => {
    console.log("hover on country: ", geographies[countryIndex])
  }
  const handletool = (e,d) => {
  ...
  const handleMarkerHover = i => {
    console.log("hover Marker: ", cities[i])
  }

  return (
    <svg width={ 1600 } height={ 950 } viewBox="0 0 800 450">
      <g className="countries">
        {
          geographies.map((d,i) => (
            <path
              ...
              className="country"
              fill={ `rgba(38,50,56,${ 1 / geographies.length * i*2})` }
              stroke="#FFFFFF"
              strokeWidth={ 0.1 }
      ...
      <g className="markers">
        {
          cities.map((city, i) => (
            <circle
              ...
              onMouseEnter={()=>handleMarkerHover(i)}
